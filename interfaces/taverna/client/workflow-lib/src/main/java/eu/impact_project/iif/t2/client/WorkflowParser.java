...
public class WorkflowParser extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request,
        ...
        while (htmlFormItems.get("currentTab" + k) != null) {
            session.setAttribute("currentTab" + k, htmlFormItems.get("currentTab" + k));
            k++;
        }
        ...
        if (htmlFormItems.get("selectGroup") != null) {
            ...
            while (htmlFormItems.get("MyExpGroup" + i) != null) {
                request.setAttribute("MyExpGroup" + i, htmlFormItems.get("MyExpGroup" + i));
                i++;
            }

            int j = 0;
            while (htmlFormItems.get("MyExpWorkflow" + j) != null) {
                request.setAttribute("MyExpWorkflow" + j, htmlFormItems.get("MyExpWorkflow" + j));
                j++;
            }

            RequestDispatcher rd0 = getServletContext().getRequestDispatcher("/GroupSelector");
        ...
        // here is the original servlet functionality
            ...
            while (htmlFormItems.get("file_workflow" + i) != null
                ...
                if (workflowFile != null && !workflowFile.equals("")) {
                    ...
                    String urlString = "http://www.myexperiment.org/workflow.xml?id=" + workflowUrl + "&elements=content";
